...
namespace CharEngine {
    class ObjLoader {
    public:
        //returns the loaded actor names
        static std::vector<std::string> LoadObj(string path, string actorName = "", Vector3 position = Vector3(0, 0, 0), ActorType type = ActorType::Common) {
            //create the default texture for parts without texture assigned in .mtl file
            if (Global::textures.size() <= 0) {
                Texture emptyTexture;
                Global::textures.push_back(emptyTexture);
            }
            ...
            if (file.is_open()) {
                ...
                while (getline(file, line)) {
                    if (line[0] == 'n') {
                        ...
                        bool t = true;
                        auto end = std::end(Global::textures);
                        for (auto tex = std::begin(Global::textures); tex != end; ++tex) {
                            if (tex->texturePath == texturePath) {
                                mat.textureId = tex - Global::textures.begin();
                                t = false;
                                break;
                            }
                        }
                        ...
                        if (t) {
                            ...
                            if (success) {
                                ...
                                Global::textures.push_back(tx);
                                mat.textureId = Global::textures.size() - 1;
                            }
                        }
                    }
                }
    ...
    private:
        ...
        static float getNext(int* start, string* line, char breakpoint = ' ') {
            while ((*line)[*start] != breakpoint) { (*start)++; }
            (*start)++;
            return atof(line->c_str() + *start);
        }

        static std::string directoryOf(const std::string& fname) {
